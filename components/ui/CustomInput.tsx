...
const CustomInput = ({ label, variant, type, placeholder, value, onChange, error, ...props }: CustomInputProps) => {
    return (
        <div className="flex flex-col gap-2 relative">
            ...
            <input type={type} className={`peer border border-border w-full px-4 py-3 placeholder:text-text-muted/50 bg-transparent placeholder-transparent focus:border-accent focus:outline-none transition-colors duration-300 ${error ? "border-red-500" : ""}`} placeholder={placeholder} value={value} onChange={onChange} {...props} />
